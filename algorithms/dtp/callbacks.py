...
from beyond_backprop.networks.layers import get_all_forward_activations
from beyond_backprop.networks.network import Network
from beyond_backprop.utils.utils import named_trainable_parameters
...
if TYPE_CHECKING:
    ...
    from beyond_backprop.algorithms.dtp import DTP
...
class CompareToBackpropCallback(Callback):
    ...
    def __init__(self, temp_beta: float = 0.005) -> None:
        ...
        """
        super().__init__()
        self.last_batch: tuple[Tensor, Tensor] | None = None
    ...
    def on_train_batch_start(
        ...
        if batch_idx == 0:
            x, y = batch
            self.compute_and_log(pl_module, trainer, x, y)

    def on_train_epoch_end(
        self, trainer: Trainer, pl_module: LightningModule, unused=None
    ) -> None:
        from beyond_backprop.algorithms.algorithm import Algorithm

        if not isinstance(pl_module, Algorithm):
        ...
        for param_name, param in named_trainable_parameters(pl_module.forward_network):
    ...
    def compute_and_log(
        self, pl_module: LightningModule, trainer: Trainer, x: Tensor, y: Tensor
    ) -> None:
        x = x.to(pl_module.device)
        y = y.to(pl_module.device)
        ...
        if trainer.logger is not None:
            ...
            for parameter_name, distance in distances.items():
                pl_module.log(f"{prefix}/distance/{parameter_name}", distance)
            for parameter_name, angle in angles.items():
                pl_module.log(f"{prefix}/angle/{parameter_name}", angle)
...
def get_backprop_grads(model: Algorithm | Network, x: Tensor, y: Tensor) -> dict[str, Tensor]:
    # Get the normal backprop loss and gradients:
    if isinstance(model, Algorithm):
        forward_net = model.forward_network
    else:
        forward_net = model
    # Clear out the gradients.
    forward_net.zero_grad()

    logits = get_all_forward_activations(forward_net, x, allow_grads_between_layers=True)[-1]
    backprop_loss = F.cross_entropy(logits, y)

    # TODO: Use the functional form of autograd instead of calling loss.backward.
    # torch.autograd.grad(backprop_loss, forward_net.parameters(), )
...
def get_dtp_grads(
    model: DTP | ExampleAlgorithm, x: Tensor, y: Tensor, temp_beta: float = 0.005
) -> dict[str, Tensor]:
    forward_net = model.forward_network
    forward_net.zero_grad()
    ...
    if isinstance(forward_output, dict):
        ...
        forward_output["layer_losses"]
    else:
        forward_loss = sum(forward_output)
...
def temporarily_change_beta(model: DTP | ExampleAlgorithm, temp_beta: float = 0.005):
