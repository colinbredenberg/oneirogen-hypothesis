...
from beyond_backprop.algorithms.dtp.feedback_networks.resnet import InvertedBasicBlock
...
) -> tuple[float, float] | dict[str, tuple[float, float] | Any]:
    """Computes distance and angle between the feedforward and feedback weights.

    TODO: Need to fix this function, so it always returns the metrics in the same, unified format.
    """
...
def _compute_dist_angle_residual(forward_module: BasicBlock, backward_module: InvertedBasicBlock):
    ...
    if len(forward_module.shortcut) > 0:  # Non-identity shortcut
        metrics[4] = compute_dist_angle(
            forward_module.shortcut.conv, backward_module.shortcut.conv
        )
...
) -> dict[Any, tuple[float, float] | Any]:
