...
def make_stacked_feedback_training_figure(
    all_values: list[Tensor | np.ndarray | Sequence[Sequence[Any]]],
    row_titles: list[str],
    title_text: str,
    ...
    `all_values` should contain a sequence of list of lists. (a list of "metric_values"). Each
    "metric_values" should contain the value of a metric, for each layer, for each iteration.
    `row_titles` should contain the name associated with each item in `all_values`. `title_text` is
    the name of the overall figure.
    """
    all_values = [
        [
