...
from beyond_backprop.utils.utils import repeat_batch, split_batch
...
def get_feedback_loss(
    *,
    ...
    if has_maxunpool2d(feedback_layer):
        r = feedback_layer(y, maxpool_indices)
    else:
        r = feedback_layer(y)
    ...
    for i in range(noise_samples):
        ...
        with torch.cuda.stream(streams[i]) if use_separate_streams else nullcontext():
            # 2- Perturb x <-- x + noise and redo x--> y --> r
            dx = noise_scale * torch.randn_like(x)
            if maxpool_indices is None:
                with torch.no_grad():
                    y_noise = forward_layer(x + dx)
                r_noise = feedback_layer(y_noise)
            else:
                # TODO: DO we also get the perturbed maxpool indices?
                with torch.no_grad():
                    y_noise, noisy_maxpool_indices = forward_layer(x + dx)
                r_noise = feedback_layer(y_noise, noisy_maxpool_indices)

            # Distance between `r` and the reconstructed `r` after x perturbation.
            dr = r_noise - r

            # 3- Perturb y <-- y + noise and redo y --> r
            dy = noise_scale * torch.randn_like(y)
            if maxpool_indices is None:
                r_noise_y = feedback_layer(y + dy)
            else:
                assert not maxpool_indices.requires_grad
                r_noise_y = feedback_layer(y + dy, maxpool_indices)
