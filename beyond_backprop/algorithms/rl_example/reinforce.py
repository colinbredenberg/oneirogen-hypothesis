...
class ExampleRLAlgorithm(Algorithm[FcNet, EpisodeBatch], LightningModule):
    ...
    def forward(
        ...
        # TODO: Don't we need to take care of normalization for the action space at some
        # point?
        ...
        if not observations.is_nested:
            ...
            # (normalized) log probability of the selected action
            ...
            action_log_probabilities = torch.nested.as_nested_tensor(
                [dist.log_prob(a) for dist, a in zip(distributions, actions.unbind())]
            )
...
def main():
    ...
    network = fcnet_for_env(env.observation_space, env.action_space)
