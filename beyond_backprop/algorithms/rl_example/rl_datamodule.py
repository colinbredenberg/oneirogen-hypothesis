...
from beyond_backprop.utils.types import StageStr

from beyond_backprop.algorithms.rl_example.types import (
    Actor,
    ActorOutputDict,
    Episode,
    EpisodeBatch,
)
from beyond_backprop.algorithms.rl_example.dataset import RlDataset
from beyond_backprop.algorithms.rl_example.utils import ToTensorsWrapper
...
class RlDataModule(
    ...
    def set_actor(self, actor: Actor[Tensor, Tensor, ActorOutputDict]) -> None:
    ...
    def val_dataloader(self) -> Iterable[EpisodeBatch[ActorOutputDict]]:
    ...
    def test_dataloader(self) -> Iterable[EpisodeBatch[ActorOutputDict]]:
...
def custom_collate_fn(episodes: list[Episode[ActorOutputDict]]) -> EpisodeBatch[ActorOutputDict]:
    ...
    return EpisodeBatch(
        ...
        actor_outputs=type(episodes[0]["actor_outputs"])(
            **{
                key: torch.nested.as_nested_tensor([ep["actor_outputs"][key] for ep in episodes])
