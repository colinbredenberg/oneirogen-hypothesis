...
class VAE(ImageClassificationAlgorithm[WakeSleepNetworkType]):
    ...
    def shared_step(
        ...
        #calculate the loss
        total_likelihood_gen = - self.network.hparams.sigma_gen**2 * self.network.gen_log_prob(skip_last=True, differentiable = True)
        KL_divergence = self.network.ts[-1].standard_normal_KL_divergence()
        self.loss = torch.mean(total_likelihood_gen + KL_divergence)
        ...
        if training:
            ...
            self.log("gen_loss", self.loss, prog_bar = True)
