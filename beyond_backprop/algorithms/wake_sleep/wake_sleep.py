...
class WakeSleep(ImageClassificationAlgorithm[WakeSleepNetworkType]):
    ...
    def shared_step(
        ...
        if optimizer_idx == 0:
            ...
            total_likelihood_gen = -self.network.gen_log_prob()
            ...
            self.pre_grad_gen = torch.mean(total_likelihood_gen)#torch.mean(reward * total_likelihood_gen)
            batch_loss = self.pre_grad_gen
            #self.pre_grad_gen.backward()
            #self.gen_optim.step()
        ...
        if optimizer_idx == 1:
            ...
            logits = self.network.ts[-1].gen_output
            ...
            batch_loss = self.pre_grad_inf

        logits = F.one_hot(y).float()
        if not(training):
            batch_loss = 0.
