...
# @dataclass_transform()
# def _builds(type_or_callable: Callable[P, T], *args, **kwargs) -> type[BuildsWithSig[type[T], P]]:
#     kwargs.setdefault("zen_dataclass", {}).setdefault(
#         "cls_name", type_or_callable.__qualname__ + "Config"
#     )
#     kwargs.setdefault("populate_full_signature", True)

#     if not kwargs.get("partial"):
#         builds_bases = list(kwargs.get("builds_bases", []))
#         if not any(issubclass(base, CallableConfig) for base in builds_bases):
#             builds_bases.append(CallableConfig)
#             kwargs["builds_bases"] = tuple(builds_bases)

#     config_dataclass = _builds(type_or_callable, *args, **kwargs)
#     if type_or_callable not in class_to_config_class:
#         class_to_config_class[type_or_callable] = config_dataclass
